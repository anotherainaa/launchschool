...
# Why does it produce the weird error?
. in the method tasks = tasks.select think itself a local variable rathen
. we are unintentionally referencing the local variable `tasks`.
. Since it has not been assigned a value, the value is `nil`.
. as refelected by the error message.

# LS notes
. the getter method `tasks` is shadowed by the local variable `tasks`
. Ruby style guide: avoid shadowing methods with local variables
