...
# Note
. I changed the serial number to be a protected method.
. This allows the serial number to be compared within the class
. but remains private from outside of the class.
. We do this because we need to compare the full serial number
. and not just the abbreviated number
. We don't want to add any public method that exposes the serial number

# LS notes
. Protectd method work just like private methods.
. But they are accessible from within the class and may be invoked
. with an explicit caller.
. Private methods cannot be invoked with an explicit caller, even inside
. their own class. The explicit caller on line 56 is `other`.
