...
var _ = function(element) {
  var u = {
    ...
    without: function(...args) {
      ...
      let indexes = args.map(item => element.indexOf(item));
    ...
    sample: function(quantity) {
      ...
      if (!quantity) return getRandom();
      
      while (sampled.length !== quantity) {
    ...
    findWhere: function(obj) {
      ...
      for (let i = 0; i < element.length; i++) {
        let currentObj = element[i];
        let objectKeys = Object.keys(currentObj);
        let objectValues = Object.values(currentObj);
        if (keys.every(key => objectKeys.includes(key)) && values.every(value => objectValues.includes(value))) {
          return currentObj;
        }
      }
      return;
    ...
    where: function(obj) {
      ...
      for (let i = 0; i < element.length; i++) {
        ...
        for (const [key1, value1] of Object.entries(currentObj)) {
          for (const [key2, value2] of Object.entries(obj)) {
            if (key1 === key2 && value1 === value2) {
              filteredArr.push(currentObj);
            }
          };
        };
      };
